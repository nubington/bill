...
namespace bill
{
    class SimpleButton : BaseObject
    {
        ...
        public static void UpdateAll(MouseState mouseState)
        {
            foreach (SimpleButton button in allButtons)
            {
                button.triggered = false;

                if (!pressing && button.Rectangle.Contains(mouseState.X, mouseState.Y) && mouseState.LeftButton == ButtonState.Pressed)
                {
                    pressing = true;
